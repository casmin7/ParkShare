        ...
        for fd in cy_feat_data:
            ...
            if abs(final_grid_diff) > 5.0:
                misaligned_list.append({
                    ...
                    'raw': fd['angle'],
                    'snap': best_snap,
                    'diff': final_grid_diff
                })
                
        print(f"Total misaligned remaining: {len(misaligned_list)}")
        for m in misaligned_list:
            print(f"  ID={m['id']} Num={m['numar']} Bat={m['baterie']} Raw={m['raw']:.1f}° Snapped={m['snap']:.1f}° GridDiff={m['diff']:.1f}°")
