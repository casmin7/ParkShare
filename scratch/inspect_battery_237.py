...
def main():
    ...
    print(f"Battery 237 features: {len(b237)}")
    b237.sort(key=lambda x: int(x['properties'].get('numar', 0)) if x['properties'].get('numar', '').isdigit() else 999)
    ...
    for feat in b237:
        ...
        print(f"ID={p.get('id')} Num={p.get('numar')} Centroid=[{lat:.6f}, {lng:.6f}] Angle={angle:.1f}°")
